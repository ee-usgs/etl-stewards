...
exec etl_helper_r_detect_qnt_lmt.drop_indexes('stewards');

prompt populating stewards r_detect_qnt_lmt
truncate table r_detect_qnt_lmt_swap_stewards;

insert /*+ append parallel(4) */
  into r_detect_qnt_lmt_swap_stewards(data_source_id, data_source, station_id, site_id, event_date, activity, analytical_method,
...
commit;
select 'Building r_detect_qnt_lmt_swap_stewards complete: ' || systimestamp from dual;

prompt building stewards r_detect_qnt_lmt indexes
exec etl_helper_r_detect_qnt_lmt.create_indexes('stewards');
