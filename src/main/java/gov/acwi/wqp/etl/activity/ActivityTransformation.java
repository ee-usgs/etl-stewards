...
import gov.acwi.wqp.etl.activity.domain.ArsResultRowMapper;
import gov.acwi.wqp.etl.activity.domain.Activity;
import gov.acwi.wqp.etl.activity.domain.ActivityProcessor;
...
public class ActivityTransformation {
	...
	@Autowired
	@Qualifier("setupActivityTable")
	private Tasklet setupActivityTable;
...
//	private Flow buildActivityIndexesFlow;
	...
	public JdbcCursorItemReader<ArsResult> activityReader() {
		return new JdbcCursorItemReaderBuilder<ArsResult>()
				...
				.sql("select row_number() over () activity_id, a.* from (select distinct "
						...
						+ "  ars_result.monitoring_location_identifier,"
						...
						+ "  station_swap_stewards.data_source_id,"
						+ "  station_swap_stewards.data_source,"
	...
	@Bean
	public Step setupActivityTableStep() {
		return stepBuilderFactory.get("setupActivityTableStep")
				.tasklet(setupActivityTable)
				.build();
	}
