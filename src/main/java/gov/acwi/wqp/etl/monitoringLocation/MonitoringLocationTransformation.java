...
import gov.acwi.wqp.etl.monitoringLocation.domain.ArsStationRowMapper;
import gov.acwi.wqp.etl.monitoringLocation.domain.MonitoringLocation;
import gov.acwi.wqp.etl.monitoringLocation.domain.MonitoringLocationProcessor;
...
public class MonitoringLocationTransformation {
	...
	@Autowired
	@Qualifier("setupMonitoringLocationTable")
	private Tasklet setupMonitoringLocationTable;
	...
	public JdbcCursorItemReader<ArsStation> monitoringLocationReader() {
		return new JdbcCursorItemReaderBuilder<ArsStation>()
				...
				.sql("select ars_station.*, ars_org_project.*, ars_site_type_to_primary.primary_site_type from ars_station join ars_org_project on 1=1 left join ars_site_type_to_primary on ars_station.monitoring_location_type_name = ars_site_type_to_primary.site_type")
	...
	@Bean
	public Step setupMonitoringLocationTableStep() {
		return stepBuilderFactory.get("setupMonitoringLocationTableStep")
				.tasklet(setupMonitoringLocationTable)
				.build();
	}
	...
	public Flow monitoringLocationFlow() {
		return new FlowBuilder<SimpleFlow>("monitoringLocationFlow")
				.start(setupMonitoringLocationTableStep())
